...
import { usCitiesCoordinates } from './us-cities-coordinates';

/**
 * Helper function to get city or state center coordinates
 * First tries to find city coordinates, falls back to state center
 */
export function getCityCoordinates(city: string, state: string): { lat: number; lng: number } | null {
  ...
  if (city) {
    ...
    if (usCitiesCoordinates[cityKey]) {
      return usCitiesCoordinates[cityKey];
    }
  }
